...
class PageDetailsPage extends StatelessWidget {
  ...
  PageDetailsPage({this.pageModel, required this.type, required this.title, this.url, Key? key})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (type == 1) {
      ...
      if (pageModel == null) logic.getLicense();
    } else if (type == 4) {
      logic.getPageDetails(pageModel?.id);
    } else {
      log('type11 ===== $type');
      if (url?.contains('shipping-and-payment') == true) {
      } else if (url?.contains('privacy-policy') == true && url?.contains('blogs') == false) {
        type = 1;
        logic.getPrivacyPolicy();
      } else if (url?.contains('refund-exchange-policy') == true &&
          url?.contains('blogs') == false) {
        type = 2;
        logic.getRefundPolicy();
      } else if (url?.contains('terms-and-conditions') == true && url?.contains('blogs') == false) {
        type = 3;
      ...
      } else if (url?.contains('license') == true && url?.contains('blogs') == false) {
        type = 7;
        logic.getLicense();
            ...
            : RefreshIndicator(
                ...
                child: (logic.pageModel?.contentWithoutTags?.length ?? 0) < 1
                    ? SizedBox(
                        height: 700.h,
                        child: Center(
                          child: CustomText('نعتذر ، لا يوجد محتوى لهذة الصفحة حاليا'.tr),
                        ),
                      )
